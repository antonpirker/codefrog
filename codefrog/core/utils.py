...
def date_range(start_date, end_date):
    start_date = (
        start_date.replace(hour=0, minute=0, second=0, microsecond=0).date()
    ...
    df = df.resample(frequency).agg(
        {
            ...
            "github_pull_requests_cumulative_age": np.sum,
...
class GitHub:
    ...
    def __init__(self, installation_id=None, code=None, state=None):
        logger.debug(
            f"---> GitHub __init__ installation_id={installation_id}, code={code}, state={state}"
        )
    ...
    def get_access_token(self, installation_id):
        ...
        if not token:
            logger.warn(
                f"Could not get access token for installation id {installation_id}: {out}"
            )
    ...
    def get_repository(self, repository_full_name):
        logger.debug(
            f"---> GitHub get_repository repository_full_name={repository_full_name}"
        )
    ...
    def _get_issues(self, repo_owner, repo_name, params):
        ...
        url = f"{self.api_base_url}{list_issues_url}?%s" % urllib.parse.urlencode(
            params
        )
    ...
    def get_releases(self, repo_owner, repo_name):
        ...
        url = f"{self.api_base_url}{list_releases_url}?%s" % urllib.parse.urlencode(
            params
        )
    ...
    def _get_pull_requests(self, repo_owner, repo_name, params):
        ...
        url = (
            f"{self.api_base_url}{list_pull_requests_url}?%s"
